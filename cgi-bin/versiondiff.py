...
class Changes:
    ...
    def search(self, s):
        for key, val in self.data.items():
            if s == key or s == val:
                self.update(key,val)
            elif s in key or s in val:
                self.update(key,val)

    def update(self, two, three):
        self.results[two] = three  #results[python2 syntax] = python3 syntax

    def html_output(self):
        if not self.results:
            return 'Nothing found relevant to version differences!'
        stringer = ''
        for key, val in self.results.items():
            stringer += '<font color=red>Python 2.x Syntax:</font> {} <br />'.format(key)
            stringer += '<font color=green>Python 3.x Syntax:</font> {} <br /><br />'.format(val)
        return stringer
